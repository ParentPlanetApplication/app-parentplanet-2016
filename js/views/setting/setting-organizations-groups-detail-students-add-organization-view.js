	...
	*/
	...
	var buildEmails = function () {
		...
		var createEmails = function ( type, selectedOrgId, groupId, datas, customListId, customListName, recipients ) {
			...
			var EmailQuery = Parse.Object.extend( "Email", {}, {
				query: function () {
			...
			$.each( Object.keys( datas ), function ( i, item ) {
				...
				query.find( {
					success: function ( results ) {
						var isNewMail = true;
						$.each( results, function ( i, email ) {
							isNewMail = false;
							var recipients_ = email.get( 'recipientAddress' );
							for ( var recipientIndex = 0; recipientIndex < recipients.length; recipientIndex++ ) {
								if ( recipients_.indexOf( recipients[ recipientIndex ] ) === -1 ) {
									recipients_.push( recipients[ recipientIndex ] );
								}
							}
							email.set( 'recipientAddress', recipients_ );
		...
		var keys = Object.keys( hashEmails );

		$.each( keys, function ( i, key ) {
			dataObj = hashEmails[ key ];
			_emailList = [];
				...
				success: function ( results ) {
					$.each( results, function ( index, result ) {
						if ( result.get( 'isEmailDelivery' ) ) {
							_emailList.push( result.get( 'email' ) );
						}
					} );
				},
				error: function ( error ) {
        ...
        query.find({
          success: function(results) {
            $.each( results, function ( index, result ) {
              var email = result.get( 'givenAccessUserEmail' );
  						if ( email && _emailList.indexOf(email) === -1) {
          ...
          _updateInfoCustomListToEmail( dataObj.groupId ).then( function ( d ) {
  					if ( d != undefined ) {
  						// window.customListIdHomeworkforOrg = d[ 0 ].customListId;
  						// window.customListNameHomeworkforOrg = d[ 0 ].customListName;
  						// console.log(window.customListIdHomeworkforOrg);
  						// console.log(window.customListNameHomeworkforOrg);
  						createEmails( dataObj.type, dataObj.selectedOrgId, dataObj.groupId, dataObj.dataId, d[ 0 ].customListId, d[ 0 ].customListName, _emailList );
  					}
	...
	var processResponses = function () {
		...
		var getParent = function ( allStudentData ) {
			...
			for ( var i = 0; i < allStudentData.length; i++ ) {
				...
				query.equalTo( "childId", studentId );
				query.find( {
					success: function ( results ) {
						for ( var j = 0; j < results.length; j++ ) {
							...
							var x = parentIdArray.indexOf( parentId );
							if ( x == -1 ) {
								parentIdArray.push( parentId );
								parentObjectArray.push( {
									parent: parentId,
									children: [ childId ]
								} )
							} else {
								parentObjectArray[ x ].children.push( childId );
							}
						}
						var numChildren = 0;
						$.each( parentObjectArray, function ( m, o ) {
							numChildren = numChildren + o.children.length;
						} ); //eo .each
						if ( numChildren == allStudentData.length && !flag ) {
							flag = true;
		...
		var addToCustomList = function ( parentObjectArray, parentIdArray ) {
			...
			query.find( {
					...
					for ( var i = 0; i < results.length; i++ ) {
						...
						for ( var j = 0; j < parentIdArray.length; j++ ) {
							...
							if ( recipient.length == 0 ) {
								recipientList.push( parentObjectArray[ j ] );
							} else {
								for ( var m = 0; m < parentObjectArray[ j ].children.length; m++ ) {
									var childId = parentObjectArray[ j ].children[ m ];
				...
				for ( var m = 0; m < parentIdArray.length; m++ ) {
					...
					addOldEvents( selectedOrgGroupId, parentObjectArray[ m ].children, parentId, m );
					addOldHomework( selectedOrgGroupId, parentObjectArray[ m ].children, parentId );
				}
			} )
		}; //eo addToCustomList

		var addOldEvents = function ( groupId, childIdList, parentId, index ) {
			...
			query.find( {
				success: function ( results ) {
					...
					$.each( results, function ( i, event ) {
						...
						isUserEventRelationExist( parentId, eventId, groupType, groupId );
				...
				if ( index == parentIdArray.length - 1 ) {
					buildEmails();
				}
			} ); //eo query.find
			var createUserEventRelation = function ( parentId, eventId, groupType, groupId ) {
			...
			var isUserEventRelationExist = function ( parentId, eventId, groupType, groupId ) {
				var UserEventRelation = Parse.Object.extend( "UserEventRelation", {}, {
					query: function () {
						return new Parse.Query( this.className );
					}
              ...
              if(!eventRelation.get('groupId')) {
                eventRelation.set( "groupId", groupId );
							...
							createUserEventRelation( parentId, eventId, groupType, groupId );
						}
					},
					error: function ( error ) {
						console.log( 'Error: ' + JSON.stringify( error ) );
			...
			var createEmails = function ( event, groupId, orgId, parentId ) {
				console.log( 'create email from event' );
				var data = {
				...
				_updateInfoCustomListToEmail( groupId ).then( function ( d ) {
					if ( d != undefined ) {
						window.customListIdEventforOrg = d[ 0 ].customListId;
						window.customListNameEventforOrg = d[ 0 ].customListName;
					}
				} ); // get information user custom list
				...
				query.find( {
					success: function ( results ) {
						...
						if ( parent.get( "isEmailDelivery" ) ) {
							...
							email.set( "customListId", window.customListIdEventforOrg );
							email.set( "customListName", window.customListNameEventforOrg );
							email.save();
						}
					},
					error: function ( error ) {
			...
			var updateEmails = function ( event, groupId, orgId, parentId, email ) {
				console.log( 'update email from event' );
				var data = {
					"allDay": event.get( "isAllDay" ),
					"end": event.get( "endDateTime" ),
					"location": event.get( "location" ),
		...
		var addOldHomework = function ( groupId, childIdList, parentId ) {
			var parentId = parentId;
			var childIdList = childIdList;
			var Today = new Date();
			...
			query.find( {
				success: function ( results ) {
					for ( var i = 0; i < results.length; i++ ) {
						...
						isUserHomeworkRelationExist( parentId, homeworkId, groupType );
			...
			var createEmails = function ( homework, groupId, orgId, parentId ) {
				console.log( 'create email from home' );
				var data = {
				...
				query.find( {
					success: function ( results ) {
						...
						if ( parent.get( "isEmailDelivery" ) ) {
							...
							email.set( "customListId", window.customListIdHomeworkforOrg );
							email.set( "customListName", window.customListNameHomeworkforOrg );
							email.save();
						}
					},
					error: function ( error ) {
			...
			var updateEmails = function ( homework, groupId, orgId, parentId, email ) {
				console.log( 'update email from homework' );
				var data = {
					"due": homework.get( "dueDate" ),
					"note": homework.get( "note" ),
					"repeat": homework.get( "repeat" ),
