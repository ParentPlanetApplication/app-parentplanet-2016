 ...
 * sxm: modified so that if there is a subhandle option, bind event to it
 * but keep handle because we need it to define the container for dragging
 */
Draggabilly.prototype.setHandles = function() {
  ...
  //sxm add in subhandles for dragging on a portion of a handle
  this.subhandles = this.hasSubhandles ? this.element.querySelectorAll( this.options.handle ) : [ this.element ];
  this.hasSubhandels = false;
  if(this.options.subhandle) {
  	this.hasSubhandles = true;
  	this.subhandles = this.element.querySelectorAll( this.options.subhandle );
  	len = this.subhandles.length;
  }
  for ( i=0; i < len; i++ ) { //len defined above
  	//use either the handle or subhandle for event binding
    var handle = (this.hasSubhandles ? this.subhandles[i] : this.handles[i]);
...
Draggabilly.prototype.measureContainment = function() {
  ...
  //sxm use the handle element
  var _element = this.element; //to restore afterwards
  this.element = (this.handles.length > 0 ? this.handles[0] : this.element);
  ...
  var margins = this.options.margins; //sxm modify containerSize to suit
  if(margins) {
  	this.containerSize.height = (margins.height ? this.containerSize.height - margins.height : this.containerSize.height);
  	this.containerSize.width = (margins.width ? this.containerSize.width - margins.width : this.containerSize.width);
  }
