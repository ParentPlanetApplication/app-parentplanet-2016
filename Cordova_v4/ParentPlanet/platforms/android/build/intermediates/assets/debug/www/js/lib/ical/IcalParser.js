...
//Simple as that.
  ...
  Ical = window.Ical = function(name) {
    this.version = '';
    this.prodid = '';
    this.events = [];
    this.todos = [];
  ...
  _.extend(Ical.prototype, {
    parseIcal: function(icsString){
      this.version=this.getValue('VERSION',icsString);
      this.prodid=this.getValue('PRODID',icsString);
    ...
    parseVfreebusy: function(vfreeString){
      ...
      this.freebusys[this.freebusys.length]=freebusy;
    },
    parseVjournal: function(vjournalString){
      ...
      this.journals[this.journals.length]=journal;
    },
