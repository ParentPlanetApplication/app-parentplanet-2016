...
- (void) modifyEventWithOptions:(CDVInvokedUrlCommand*)command {
    ...
    [self.commandDelegate runInBackground: ^{
        ...
        if (theEvent != nil) {
            ...
            // Remove all current recurrenceRules.
            while (theEvent.recurrenceRules.count > 0) {
                [theEvent removeRecurrenceRule:theEvent.recurrenceRules.firstObject];
            }
...
- (void) deleteEventById:(CDVInvokedUrlCommand*)command {
    ...
    NSString *lowerID = [eventID lowercaseString];
    NSError *error = NULL;
    // NOTE: as per issue #150 you can delete this event AND future events by passing span:EKSpanFutureEvents
    [self.eventStore removeEvent:theEvent span:EKSpanFutureEvents error:&error];
    
    CDVPluginResult *pluginResult = nil;
    if (error) {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:error.userInfo.description];
    } else {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:[@"Deleted " stringByAppendingString:eventID]];
    }
...
- (EKCalendar*) findEKCalendar: (NSString *)calendarName {
    NSLog(@"findEKCalendar - Calendar: %@", calendarName);
    for (EKCalendar *thisCalendar in [self.eventStore calendarsForEntityType:EKEntityTypeEvent]){
        NSLog(@"Calendar: %@ --- %@", thisCalendar.title, calendarName);
...
- (void)createEventWithOptions:(CDVInvokedUrlCommand*)command {
    ...
    [self.commandDelegate runInBackground: ^{
        ...
        if (calendarName == (id)[NSNull null]) {
            ...
            NSLog(@"Result find: %@", calendar.title);
            if (calendar == nil) {
                // create it
...
- (void) modifyRepeatEventInstance:(CDVInvokedUrlCommand*)command {
    NSDictionary* options = [command.arguments objectAtIndex:0];
    NSString* syncId      = [options objectForKey:@"syncId"];
    NSNumber* oldStartTime  = [options objectForKey:@"oldStartTime"];
    NSNumber* oldEndTime    = [options objectForKey:@"oldEndTime"];
    NSString* calendarName      = [options objectForKey:@"calendar"];
    BOOL isCanceled = [(NSNumber*)[options objectForKey:@"isCanceled"] boolValue];
    NSString* title      = [options objectForKey:@"title"];
    NSString* location   = [options objectForKey:@"location"];
    NSNumber* startTime  = [options objectForKey:@"startTime"];
    NSNumber* endTime    = [options objectForKey:@"endTime"];
    NSNumber* firstReminderMinutes    = [options objectForKey:@"firstReminderMinutes"];
    NSNumber* secondReminderMinutes    = [options objectForKey:@"secondReminderMinutes"];
    ...
    [self.commandDelegate runInBackground: ^{
        ...
        NSDate* oldStartDate;
        if (oldStartTime) {
            NSTimeInterval _nstartInterval = [oldStartTime doubleValue] / 1000; // strip millis
            oldStartDate = [NSDate dateWithTimeIntervalSince1970:_nstartInterval];
        }
        
        NSDate* oldEndDate;
        if (oldEndTime) {
            NSTimeInterval _nstartInterval = [oldEndTime doubleValue] / 1000; // strip millis
            oldEndDate = [NSDate dateWithTimeIntervalSince1970:_nstartInterval];
        }
        ...
        NSArray* events = [self findEKEventsWithTitle:nil location:nil notes:syncId startDate:oldStartDate endDate:oldEndDate calendars:calendars];
        
        CDVPluginResult *pluginResult = nil;
        
        if (events.count > 0) {
            EKEvent* theEvent = events[0];
            ...
            if (theEvent != nil) {
                ...
                if (isCanceled) {
                    ...
                    if (startTime) {
                        NSTimeInterval _nstartInterval = [startTime doubleValue] / 1000; // strip millis
                        theEvent.startDate = [NSDate dateWithTimeIntervalSince1970:_nstartInterval];
                    }
                    if (endTime) {
                        NSTimeInterval _nendInterval = [endTime doubleValue] / 1000; // strip millis
                        theEvent.endDate = [NSDate dateWithTimeIntervalSince1970:_nendInterval];
                    }
                ...
                pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"No existing event found."];
            }
        } else {
            // Otherwise return a no result error (could be more than 1, but not a biggie)
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_NO_RESULT];
